...
import { ScrollView, Text, View, Modal, TouchableNativeFeedback } from 'react-native';
...
export class FilterModal extends Component {
              ...
              <View style={sectionContainer} key={i}>
                ...
                >
                  <View style={sectionStyle} key={i + 1}>
                    <Text style={sectionTextStyle}>{_.startCase(head)}</Text>
                  </View>
                </TouchableNativeFeedback>
                      ...
                      tr.map((td, i3) => {
                        if (i === i3 && td) {
                          if (!_.includes(listedValues[i], td)) {
                            listedValues[i]
                              ? listedValues[i].push(td)
                              : (listedValues = Object.assign({ [i]: [td] }, listedValues));
                                ...
                                >
                                  <View style={iconContainer} key={i3 + 2}>
                                    {!_.includes(this.props.filter.category[i], td) && (
                                      <Icon name="check" color="#5BC4E5" size={12} />
                                    )}
                                  </View>
                                  <Text style={valueStyle} key={i3 + 3}>
                                    {td}
                                  </Text>
                                </View>
                              </TouchableNativeFeedback>
  ...
  resolveFilterExclude(categoryKey, valueKey) {
    ...
    _.includes(category[categoryKey], valueKey)
      ...
      ? category[categoryKey].push(valueKey)
      : (category = Object.assign({ [categoryKey]: [valueKey] }, category));
      ...
      <Modal animationType="slide" transparent visible={visible} onRequestClose={() => {}}>
        <View style={modalContainer}>
          ...
          <View style={containerStyle}>
            <ScrollView>{this.resolveFilter()}</ScrollView>
          </View>
        </View>
      </Modal>
    );
