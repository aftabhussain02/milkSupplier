...
export class FilterTable extends Component {
  ...
  onChangeSortBy(i) {
    ...
    const rowData = data.rowData.sort((a, b) =>
      i === 0
  ...
  resolveGroupByData(groupBy, nextFilter) {
    const rowData = [];
    this.state.data.rowData.map((tr, i) => {
      let filter = false;
          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            {headData.map((head, i) => (
              <TouchableWithoutFeedback
                ...
                onLongPress={() =>
                  this.setState(prevState => ({
                    groupBy: !_.includes(prevState.groupBy, i)
                      ? [...prevState.groupBy, i]
                      : prevState.groupBy,
                  }))
                }
                      ...
                      <View key={i} style={{ minHeight: 40, flexDirection: 'row' }}>
                        ...
                        >
                          <Text style={{ justifyContent: 'center' }}>
                            {headData[i]}: {v} - {group.items.length} Item
                          </Text>
                        </View>
                      </View>
                    );
                        ...
                        {v2.map((v3, key) => {
                          if (key == 5) {
                            return (
                              ...
                              >
                                <Text style={element} onPress={() => onViewPress(v3)}>
                                  View
                                </Text>
                                <Text style={elementDif}>|</Text>
                                <Text style={element} onPress={() => onEditPress(v3)}>
                                  Edit
                                </Text>
                              </View>
                            );
              ...
              if (!filter) {
                        ...
                        if (i === i3) {
                          return (
                            <View style={elementContainer}>
                              <Text style={element} onPress={() => onViewPress(td)}>
                                View
                              </Text>
                              <Text style={elementDif}>|</Text>
                              <Text style={element} onPress={() => onEditPress(td)}>
                                Edit
                              </Text>
                            </View>
                          );
                ...
                return (
                  <View style={[tdContainer, i === 0 && { borderLeftWidth: 0 }]} key={i2}>
                    {tr.map((td, i3) => {
                      if (i === i3) {
                        return <Text style={tdStyle}>{td}</Text>;
                      }
                      return <View />;
                    })}
                  </View>
